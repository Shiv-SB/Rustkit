...
3. **Benchmarks → `bun run bench`** or `bun run bench vector` to filter by name.

There is no separate lint/typecheck step configured. Rust has no clippy/rustfmt config checked in.

## Architecture

- **`crates/rustkit-core`** — Pure Rust algorithms. Each operation is its own file under `src/<module>/<operation>.rs`. Modules: vector, matrix, stats, sort, search, bitset, string, geohash, crypto, quantile.
...
- **`src/packages/*.ts`** — TypeScript wrappers that validate inputs then call `native.symbols.*`. One file per module, re-exported from `src/index.ts`.
